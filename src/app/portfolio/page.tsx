            ...
            <div className="max-w-7xl mx-auto px-6 sm:px-12 md:px-20 lg:px-32 pb-32 space-y-32">
                {categories.map((category) => (
                    <section key={category.id} id={`category-${category.id}`} className="scroll-mt-32">
                        <div className="mb-12">
                            <h2 className="text-4xl sm:text-5xl font-black tracking-tight text-black border-l-4 border-orange-500 pl-6">
                                {category.name}
                            </h2>
                        ...
                        {category.projects.length > 0 ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-12">
                                {category.projects.map((project) => (
                                    <div key={project.id} className="bg-white rounded-[2rem] shadow-xl overflow-hidden border border-gray-100 flex flex-col group hover:shadow-2xl hover:border-orange-500/20 transition-all duration-500">
                                            ...
                                            {project.images.length > 1 && (
                                                <div className="absolute inset-x-0 bottom-4 flex justify-center space-x-2 z-10">
                                                    {project.images.map((_, i) => (
                                                        <div key={i} className={`w-2 h-2 rounded-full ${i === 0 ? 'bg-orange-500 shadow-md shadow-orange-500/50' : 'bg-white/80'}`}></div>
                                                    ))}
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        <div className="p-8 flex-1 flex flex-col">
                                            <h3 className="text-2xl font-black text-black mb-4 grouping-tight">{project.title}</h3>
                                            <p className="text-gray-600 mb-8 leading-relaxed font-medium">{project.description}</p>
                                            {project.link && (
                                                <a href={project.link} target="_blank" rel="noopener noreferrer" className="mt-auto inline-flex items-center text-orange-500 font-bold uppercase tracking-widest text-xs hover:text-black transition-colors group/link w-fit">
                                                    Visit Project
