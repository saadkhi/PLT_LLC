        ...
        <div className="bg-white min-h-screen">
            ...
            <div className="pt-24 pb-12 px-6 sm:px-12 md:px-20 lg:px-32 text-center">
                <h1 className="text-fluid-h1 font-black uppercase tracking-tighter text-[#0A2540]">
                    Our <span className="text-cyan-500">Insights</span>
                </h1>
                ...
                <div className="max-w-screen-xl mx-auto px-6 sm:px-12 md:px-20 lg:px-32">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {insights.map((insight: any) => (
                            <div key={insight.id} className="group bg-white rounded-[2.5rem] shadow-xl shadow-gray-200/50 overflow-hidden flex flex-col border border-gray-100 transition-all duration-500 hover:scale-[1.02] hover:shadow-2xl hover:shadow-cyan-500/5">
                                ...
                                <div className="p-8 flex-1 flex flex-col">
                                    <div className="flex items-center gap-2 text-cyan-600 font-bold uppercase text-[10px] tracking-widest mb-4">
                                        <Calendar size={14} />
                                        <span>{new Date(insight.created_at).toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                                    </div>
                                    <h3 className="text-2xl font-black tracking-tight text-[#0A2540] mb-4 group-hover:text-cyan-500 transition-colors line-clamp-2 uppercase leading-tight">
                                        {insight.title}
                                    </h3>
                                    ...
                                    >
                                        <span className="text-xs font-black uppercase tracking-widest text-[#0A2540] group-hover/btn:text-cyan-500 transition-colors">Read Article</span>
                                        <div className="w-8 h-8 rounded-full border border-gray-100 flex items-center justify-center group-hover/btn:bg-cyan-500 group-hover/btn:border-cyan-500 group-hover/btn:text-white transition-all">
                                            <ArrowRight size={14} />
                                        </div>
                                    </Link>
                                </div>
